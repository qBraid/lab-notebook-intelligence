        ...
        for cell_source in cell_sources:
            ...
            if cell_type == "markdown":
                ...
                ui_cmd_response = await response.run_ui_command(
                    ...
                    {"markdown": source, "path": file_path},
                )
            elif cell_type == "code":
                source = cell_source.get("source", "")
                ui_cmd_response = await response.run_ui_command(
                    "lab-notebook-intelligence:add-code-cell-to-notebook",
                    {"code": source, "path": file_path},
                )
...
class AddMarkdownCellToNotebookTool(Tool):
    ...
    async def handle_tool_call(
        ...
        ui_cmd_response = await response.run_ui_command(
            "lab-notebook-intelligence:add-markdown-cell-to-notebook",
            {"markdown": source, "path": notebook_file_path},
        )
...
class AddCodeCellTool(Tool):
    ...
    async def handle_tool_call(
        ...
        ui_cmd_response = await response.run_ui_command(
            "lab-notebook-intelligence:add-code-cell-to-notebook",
            {"code": source, "path": notebook_file_path},
        )
...
class PythonTool(AddCodeCellTool):
    ...
    async def handle_tool_call(
        ...
        ui_cmd_response = await response.run_ui_command(
            "lab-notebook-intelligence:add-code-cell-to-notebook",
            {"code": code, "path": tool_context.get("file_path")},
        )
...
class BaseChatParticipant(ChatParticipant):
    ...
    def tools(self) -> list[Tool]:
        ...
        if chat_mode.id == "ask":
            ...
            ) in tool_selection.mcp_server_tools.items():
    ...
    async def handle_ask_mode_chat_request(
        ...
        if request.command == "newNotebook":
            ...
            ui_cmd_response = await response.run_ui_command(
                "lab-notebook-intelligence:add-markdown-cell-to-notebook",
                {"markdown": markdown, "path": file_path},
            )
            ui_cmd_response = await response.run_ui_command(
                "lab-notebook-intelligence:add-code-cell-to-notebook",
        ...
        messages = [
            {
                ...
                "content": options.get(
                    "system_prompt",
                    self.chat_prompt(chat_model.provider.name, chat_model.name),
                ),
            },
        ...
        try:
            ...
            response.stream(
                MarkdownData(
                    f"Oops! There was a problem handling chat request. Please try again with a different prompt."
                )
            )
            response.finish()
