...
class NBIConfig:
    def __init__(self, options: dict = {}):
        ...
        self.deprecated_env_config_file = os.path.join(
            sys.prefix, "share", "jupyter", "nbi-config.json"
        )
        self.deprecated_user_config_file = os.path.join(
            os.path.expanduser("~"), ".jupyter", "nbi-config.json"
        )
        ...
        if os.path.exists(self.deprecated_env_config_file):
            log.warning(
                f"Deprecated config file found: {self.deprecated_env_config_file}. Use {self.env_config_file} and {self.env_mcp_file} instead."
            )
        if os.path.exists(self.deprecated_user_config_file):
            log.warning(
                f"Deprecated config file found: {self.deprecated_user_config_file}. Use {self.user_config_file} and {self.user_mcp_file} instead."
            )
        if (
            self.env_mcp.get("participants") is not None
            or self.user_mcp.get("participants") is not None
        ):
            log.warning(
                "MCP participants configuration is deprecated. Users should use Agent mode to select MCP tools."
            )
    ...
    def using_github_copilot_service(self) -> bool:
        return (
            self.chat_model.get("provider") == "github-copilot"
            or self.inline_completion_model.get("provider") == "github-copilot"
        )
